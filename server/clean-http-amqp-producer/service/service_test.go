...
type mockAMQPProducerRepository struct {
	mock.Mock
}

func (m *mockAMQPProducerRepository) ProduceJSONMessage(ctx context.Context, event dto.Event) error {
...
func TestDoProduce(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		mockAMQPProducerRepository := new(mockAMQPProducerRepository)
		mockAMQPProducerRepository.On("ProduceJSONMessage", mock.Anything, mock.Anything).Return(nil)

		s := New(Dependencies{
			AMQPProducerRepository: mockAMQPProducerRepository,
		})

		err := s.DoProduce(mockContext, mockEventData)
